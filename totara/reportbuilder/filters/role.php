...
class rb_filter_role extends rb_filter_type {
    ...
    public function __construct($type, $value, $advanced, $region, $report) {
        ...
        if (!isset($this->options['selectchoices'])) {
            debugging("No selectchoices provided for filter '{$this->name}' in source '" . get_class($report->src) . "'", DEBUG_DEVELOPER);
            $this->options['selectchoices'] = array();
        }
        if (!isset($this->options['dataseparator'])) {
            debugging("No dataseparator provided for filter '{$this->name}' in source '" . get_class($report->src) . "'", DEBUG_DEVELOPER);
            $this->options['dataseparator'] = array();
        }
    ...
    public function get_sql_filter($data) {
        ...
        $param = rb_unique_param($this->filtertype);
        $params = array();

        if ($data['operator'] == self::OPERATOR_ASSIGNED && $data['value'] !== '') {
            if ($data['value']) {
                $sql = $DB->sql_like($field, ":{$param}");
                $params[$param] = "%{$this->options['dataseparator']}{$data['value']}{$this->options['dataseparator']}%";
            } else {
                // Assume here that we've got a zero and that means any value in the field is a match.
                $sql = "{$field} != :{$param}";
                $params[$param] = '';
            }
        } else if ($data['operator'] == self::OPERATOR_NOTASSIGNED && $data['value'] !== '') {
            if ($data['value']) {
                $sql = $DB->sql_like($field, ":{$param}", true, true, true) . " OR {$field} IS NULL";
                $params[$param] = "%{$this->options['dataseparator']}{$data['value']}{$this->options['dataseparator']}%";
            } else {
                // Assume here that we've got a zero and that means an empty field is a match.
                $sql = "({$field} = :{$param} OR {$field} IS NULL)";
                $params[$param] = '';
            }
        } else {
            $sql = ' 1=1 ';
