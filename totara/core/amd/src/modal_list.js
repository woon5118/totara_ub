    ...
    var addListTemplate = function(settings) {
        return new Promise(function(resolve) {
            constructTemplateData(settings).then(function(data) {
                ...
                TemplatesManager.render('totara_core/basket_list', data).then(function(html) {
                    ModalFactory.create({
                        body: html,
                        large: true,
                        title: data.title,
            ...
            addListTemplate(options).then(function(modal) {
                ...
                var createModalList = new Promise(function(resolve) {
                    if (options.mode === 'adder' || options.mode === 'selector') {
                        require(['totara_core/basket_list'], function(ListBase) {
                            resolve(ListBase);
                        });
                    } else {
                        require(['totara_core/view_list'], function(ListBase) {
                            resolve(ListBase);
                        });
                    }
                });
