...
define(['core/str', 'core/templates', 'core/notification', 'totara_core/paging_manager', 'core/ajax'],
function(str, templates, notification, Paging, ajax) {
    ...
    function ListManager() {
        ...
        this.rowsTemplate = 'totara_core/lists_rows';
        this.selectedItems = [];
        this.toggleLevel = '';
        this.webservice = '';
        this.widget = '';
    ...
    */
    ListManager.prototype.onCustomAction = function() { /* Null */ };
    ListManager.prototype.onHideSubLevelItems = function() { /* Null */ };
    ListManager.prototype.onItemSelected = function() { /* Null */ };
    ListManager.prototype.onItemUnselected = function() { /* Null */ };
    ListManager.prototype.onItemUpdate = function() { /* Null */ };
    ListManager.prototype.onListHierarchyLevelChange = function() { /* Null */ };
    ListManager.prototype.onListHierarchyLevelChangeExtend = function() { /* Null */ };
    ListManager.prototype.onPostRequest = function() { /* Null */ };
    ListManager.prototype.onPreRequest = function() { /* Null */ };
    ListManager.prototype.onShowSubLevelItems = function() { /* Null */ };
    ...
    ListManager.prototype.propagatedListEvents = function() {
        ...
        this.eventListenerNode.addEventListener(this.eventListener + ':action', function(e) {
            that.onCustomAction(e);
        });

        this.eventListenerNode.addEventListener(this.eventListener + ':add', function(e) {
            that.onItemSelected(e.detail.val);
        });

        this.eventListenerNode.addEventListener(this.eventListener + ':remove', function(e) {
            that.onItemUnselected(e.detail.val);
        });

        this.eventListenerNode.addEventListener(this.eventListener + ':update', function() {
            that.onItemUpdate();
        });

        this.eventListenerNode.addEventListener(this.eventListener + ':hierarchyRequest', function(e) {
        ...
        if (this.orderBy) {
            ...
            this.orderBy.addEventListener('totara_core/select_tree:changed', function() {
                that.update();
            });
        }

        if (this.toggleLevel) {
            this.eventListenerNode.addEventListener('totara_core/lists_toggle_level:changed', function(e) {
    ...
    ListManager.prototype.formatListData = function(data) {
        var active = false,
            ...
            newDataStructure = {
                ...
                'noResultsText': map.noResultsString && map.noResultsString.value ? map.noResultsString.value : '',
                'rows': [],
                'row_header': {
                    'columns': [],
                    'header': true
    ...
    ListManager.prototype.prepareActions = function(actionCallbacks, row) {
        ...
        if (typeof actionCallbacks === "function") {
            ...
            if (actions && !Array.isArray(actions)) {
                notification.exception({message: 'Expected action callback to return an Array.'});
                return [];
            }
            tmpActions = actions;
            ...
            if (actionCallbacks.length > 0) {
                for (var d = 0; d < actionCallbacks.length; d++) {
                    if (typeof actionCallbacks[d] !== "function") {
                        notification.exception({message: 'Expected array of callbacks for actions setting.'});
                        return [];
                    }
                    var callback = actionCallbacks[d];
                    var action = callback(row);
                    if (typeof action !== "object") {
                        notification.exception({message: 'Expected object to be returned by the action callback'});
                        return [];
                    }
            ...
            notification.exception({message: 'Expected either an Array of callbacks or one callback for actions setting.'});
            return [];
        }
    ...
    ListManager.prototype.prepareListIcons = function(iconCallback) {
        ...
        return new Promise(function(resolve) {
            if (!iconCallback) {
                ...
                notification.exception({message: 'Expected callback for the icons setting.'});
                resolve();
                return;
            }
            ...
            if (!Array.isArray(icons)) {
                notification.exception({message: 'Expected icons callback to return an array.'});
                resolve();
                return;
            }
            ...
            if (stringKeys.length > 0) {
                ...
                str.get_strings(stringKeys).then(function(strings) {
                    ...
                    for (var i = 0; i < icons.length; i++) {
                        ...
                        var promise = that.renderIcon(icons[i].key, icons[i].name, icons[i].classes, altText);
     ...
     * @return {Promise}
     */
    ListManager.prototype.renderIcon = function(iconKey, iconName, iconClasses, iconString) {
        var that = this;

        return new Promise(function(resolve) {
            templates.renderIcon(iconName, iconString, iconClasses).done(function(html) {
            ...
            that.renderListCount(data.total).then(function() {
                ...
                that.formatListData(data).then(function(listData) {
                    if (listData.expandWebservice) {
                        target.setAttribute('data-tw-list-webservice-expand', listData.expandWebservice);
                    }
                    if (listData.expandWebserviceArgs) {
                        target.setAttribute('data-tw-list-webservice-expand-args', JSON.stringify(listData.expandWebserviceArgs));
                    }
                    if (listData.expandTemplate) {
                        target.setAttribute('data-tw-list-template-expand', listData.expandTemplate);
                    }
                    ...
                    if (that.appendType === 'append') {
                        ...
                        templates.renderAppendByItem(that.getRowsTemplate(), listData, target, rowsSelector).then(function() {
                        ...
                        templates.renderReplace(that.getRowsTemplate(), listData, target).then(function() {
    ...
    ListManager.prototype.removeItemFromList = function(ids) {
        ...
        return new Promise(function(resolve) {
            ...
            if (that.counter) {
                var currentCount = parseInt(that.counter.querySelector('[data-tw-list-count-num]').innerHTML),
    ...
    ListManager.prototype.update = function() {
        ...
        return new Promise(function(resolve, reject) {
            that.onPreRequest();
            ajax.getDataUpdate([that.getUpdateRequestArgs()]).then(function() {
                that.onPostRequest();
                resolve();
            }).catch(function() {
                that.onPostRequest();
    ...
    var init = function(parent, data) {
        ...
        if (!data.map || typeof data.map !== "object") {
            notification.exception({message: 'You need to provide a map object for the list'});
            return null;
        }
