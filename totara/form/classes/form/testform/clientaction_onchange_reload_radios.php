...
/**
 * Onchange reload client radios action test form.
 *
 * @author Brian Barnes <brian.barnes@totaralearning.com>
 ...
 * @package totara_form
 */
class clientaction_onchange_reload_radios extends form {
    ...
    public static function get_form_test_name() {
        return 'Onchange reload client radios action test';
    }
    ...
    public static function get_form_controller() {
        return new clientaction_onchange_reload_controller_radios();
    }
    ...
    public function definition() {
        ...
        $this->model->add_clientaction(new onchange_reload($radios));
        $radios->set_attribute('horizontal', true);

        $radios = $this->model->add(new radios('radios_3', 'radios ignored values (one)', $options));
        $this->model->add_clientaction((new onchange_reload($radios))->add_ignored_value('0'));

        $radios = $this->model->add(new radios('radios_4', 'radios custom values', $options2));
        $this->model->add_clientaction(new onchange_reload($radios));

        $radios = $this->model->add(new radios('radios_5', 'radios custom values and ignored one', $options2));
        $this->model->add_clientaction((new onchange_reload($radios))->add_ignored_value('a'));

        $radios = $this->model->add(new radios('radios_6', 'radios custom values and ignore current', $options2));
        $this->model->add_clientaction((new onchange_reload($radios))->add_ignored_value('a'));
