...
Feature: Totara form checkbox element tests
  ...
  Background:
    ...
    @javascript
  Scenario: Test basic checkbox elements in Totara forms
    ...
    And "checkbox_frozen_empty" row "Value" column of "form_results" table should contain ""
    ...
    When I set the following Totara form fields to these values:
      | Basic checkbox | 1 |
      | Required basic checkbox | 1 |
      | Empty frozen checkbox | 1 |
      | checkbox with current data | 0 |
      | Frozen checkbox with current data | 0 |
