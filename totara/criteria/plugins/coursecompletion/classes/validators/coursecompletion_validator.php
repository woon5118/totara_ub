 ...
 */
class coursecompletion_validator implements criterion_validator_interface {
    ...
    public static function validate_and_set_status(criterion $criterion): int {
        ...
        if ($is_valid != $criterion->is_valid()) {
            ...
            // Not calling the criterion's save method as it will re-validate the items one by one
            $to_save = new criterion_entity($criterion->get_id());
            $to_save->status = $new_status;
            $to_save->save();        }
