...
/**
 * Action icons for export type.
 */
final class export_type_actions extends base {
    /**
     * Display data.
     *
     * @param string $value
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report) {
        ...
        if (has_capability('totara/userdata:config', \context_system::instance())) {
            $url = new \moodle_url('/totara/userdata/export_type_edit.php', array('id' => 0, 'duplicate' => $value));
            $buttons[] = $OUTPUT->action_icon($url, new flex_icon('duplicate', array('alt' => get_string('duplicate'))));

            if (export_type::is_deletable($value)) {
                $url = new \moodle_url('/totara/userdata/export_type_delete.php', array('id' => $value));
                $buttons[] = $OUTPUT->action_icon($url, new flex_icon('delete', array('alt' => get_string('delete'))));
            }

            $url = new \moodle_url('/totara/userdata/export_type_edit.php', array('id' => $value));
