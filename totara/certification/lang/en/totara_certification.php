...
$string['error:info_fixprogcompletiondatematch'] = 'The \'Program completion date\' is not displayed to users or processed internally in relation to certifications. This check was performed because, under normal circumstances, the two dates should be the same, and the problem could occur in combination with other problems. In this instance, where the problem occurred on its own, the solution is simply to copy the \'Completion date\' to the \'Program completion date\'.';
$string['error:info_fixprogincomplete'] = 'There are two known methods of resolving this problem.

It could have been caused in previous versions when a user was certified, before the recertification window opened, the user was then unassigned, and later re-assigned. If the record has a history completion record (most likely marked \'unassigned\') then this is likely the cause. In this case, you should manually edit the completion record, copying the details of the history record, then remove the history record. This will restore the user into the certification in the state that they were in before being unassigned.
