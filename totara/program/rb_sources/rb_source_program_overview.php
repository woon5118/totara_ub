...
class rb_source_program_overview extends rb_base_source {
    ...
    public function __construct($groupid, rb_global_restriction_set $globalrestrictionset = null) {
        ...
        $this->sourcesummary = get_string('sourcesummary', 'rb_source_program_overview');
        $this->sourcelabel = get_string('sourcelabel', 'rb_source_program_overview');
    ...
    protected function define_columnoptions() {
        ...
        $concat = $DB->sql_group_concat(
            \totara_program\rb_course_sortorder_helper::get_column_field_definition('course.shortname','prog_courseset.programid'),
        ...
        $from = "FROM {prog_courseset} prog_courseset
            ...
            LEFT JOIN {course_categories} course_category ON course_category.id = course.category
                WHERE prog_courseset.programid = base.programid";
        $concat = $DB->sql_group_concat(\totara_program\rb_course_sortorder_helper::get_column_field_definition(
            'course_category.idnumber', 'prog_courseset.programid'), $this->uniquedelimiter,
            'prog_courseset.sortorder ASC, prog_courseset_course.sortorder ASC');
        $columnoptions[] = new rb_column_option(
            'course',
            'id',
