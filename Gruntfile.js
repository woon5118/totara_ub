...
module.exports = function(grunt) {
    ...
    var uglifyRename = function(destPath, srcPath) {
        destPath = srcPath;
        var srcSearch = '/src/';
        var srcIndex = destPath.lastIndexOf(srcSearch)
        if (srcIndex != -1) {
            destPath = destPath.substring(0, srcIndex) +
                '/build/' +
                destPath.substring(srcIndex + srcSearch.length);
        }
    ...
    var getLessProcessedPaths = function() {
    ...
    /**
     * Generate a task implementation that runs the provided command
     *
     * @param {*} fn Function returning { cmd, args } or an array [cmd, ...args], or the value directly.
     * @returns {function}
     */
    function makeCommandTask(fn) {
        return function() {
            var opts = typeof fn == 'function' ? fn() : fn;
            if (!opts) {
                return;
            }
            if (Array.isArray(opts)) {
                opts = { cmd: opts[0], args: opts.slice(1) };
            }
            var done = this.async();
            grunt.util.spawn(
                {
    ...
    tasks.ignorefiles = function() {
        ...
        var lessProcessedPaths = getLessProcessedPaths();
