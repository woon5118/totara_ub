...
class core_dml_testcase extends database_driver_testcase {
    ...
    public function test_sql_group_concat_unique() {
        ...
        $grpconcat = $DB->sql_group_concat('valchar', ',', 'orderby DESC', true);
        ...
        if ($DB->get_dbfamily() !== 'mssql') {
            // All decent databases can order the values properly.
            $this->assertEquals('áéíóú,12345,abcde', $records[12]->grpconcat);
            $this->assertEquals('abc12,abcde', $records[24]->grpconcat);
            $this->assertEquals('a\+1_', $records[36]->grpconcat);
        }

        $grpconcat = $DB->sql_group_concat('UPPER(valchar)', ',', 'orderby DESC', true);
        $sql = 'SELECT groupid, ' . $grpconcat . ' AS grpconcat FROM {' . $tablename . '} GROUP BY groupid';
        $records = $DB->get_records_sql($sql);
        $this->assertCount(3, $records);
        $this->assertCount(3, explode(',', $records[12]->grpconcat));
        ...
        $grpconcat = $DB->sql_group_concat("COALESCE(valchar, '-')", '|', 'orderby ASC', true);
        $sql = 'SELECT groupid, ' . $grpconcat . ' AS grpconcat FROM {' . $tablename . '} GROUP BY groupid';
        $records = $DB->get_records_sql($sql);
        $this->assertCount(3, $records);
        $this->assertCount(4, explode('|', $records[12]->grpconcat));
        ...
        // Make sure tests work fine.
        $grpconcat = $DB->sql_group_concat('valint', ':', 'orderby ASC', true);
        $sql = 'SELECT groupid, ' . $grpconcat . ' AS grpconcat FROM {' . $tablename . '} GROUP BY groupid';
        $records = $DB->get_records_sql($sql);
        $this->assertCount(3, $records);
        $this->assertCount(3, explode(':', $records[12]->grpconcat));
        ...
        if ($DB->get_dbfamily() !== 'mssql') {
            // All decent databases can order the values properly.
            $this->assertEquals('2:3:4', $records[12]->grpconcat);
            $this->assertEquals('5:6:7', $records[24]->grpconcat);
            $this->assertEquals('9', $records[36]->grpconcat);
        }

        // Verify integers are cast to strings.
        $grpconcat = $DB->sql_group_concat('valtext', ':', 'orderby ASC', true);
        $sql = 'SELECT groupid, ' . $grpconcat . ' AS grpconcat FROM {' . $tablename . '} GROUP BY groupid';
        $records = $DB->get_records_sql($sql);
        $this->assertCount(3, $records);
        $this->assertCount(3, explode(':', $records[12]->grpconcat));
        ...
        if ($DB->get_dbfamily() !== 'mssql') {
            // All decent databases can order the values properly.
            $this->assertEquals($text.'1:'.$text.'2:'.$text.'3', $records[12]->grpconcat);
            $this->assertEquals($text.'6:'.$text.'7', $records[24]->grpconcat);
            $this->assertEquals($text.'8', $records[36]->grpconcat);
        }
        ...
        // Make sure the orders are independent.
        $grpconcat = $DB->sql_group_concat('valtext', ':', 'orderby ASC', true);
        $sql = 'SELECT groupid, ' . $grpconcat . ' AS grpconcat FROM {' . $tablename . '} GROUP BY groupid ORDER BY groupid DESC';
        $records = $DB->get_records_sql($sql);
        $this->assertSame(array(36, 24, 12), array_keys($records));
        $this->assertCount(3, explode(':', $records[12]->grpconcat));
        $this->assertSame(23999, strlen($records[12]->grpconcat));
        ...
        if ($DB->get_dbfamily() !== 'mssql') {
            // All decent databases can order the values properly.
            $this->assertEquals($text.'3:'.$text.'2:'.$text.'1', $records[12]->grpconcat);
            $this->assertEquals($text.'6:'.$text.'7', $records[24]->grpconcat);
            $this->assertEquals($text.'8', $records[36]->grpconcat);
        }

        try {
            $grpconcat = $DB->sql_group_concat('valint', ':', '', true);
            $sql = 'SELECT groupid, ' . $grpconcat . ' AS grpconcat FROM {' . $tablename . '} GROUP BY groupid';
            $DB->get_records_sql($sql);
            $this->fail('Exception expected if $orderby missing');
