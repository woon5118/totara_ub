...
define([ 'core/mustache',
    ...
    function(mustache, $, webapi, str, notification, coreurl, config, storage, event, flexicon, log, UserDate, Truncate) {

        // Totara: Make mustache escaping compatible with Totara s() function.
        //         This is required for clean_string() security improvements.
        mustache.escape = function(text) {
            ...
            text = String(text).replace(/[&<>"'\/`=]/g, function(ch) {
                return entityMap[ch];
            });

            text = text.replace(/&amp;#([0-9]+|x[0-9a-fA-F]+);/g, '&#$1');
         ...
         *
         * Totara: there used to be 'themeName' parameter, but it was removed because only the current theme is allowed
         *
         * @method doRender
         * @private
        ...
        var doRender = function(templateSource, context) {
            if (arguments.length > 2) {
                log.debug('template rendering cannot accept themeName parameter');
            }
        ...
        var getTemplate = function(templateName) {
            ...
            var promise = webapi.call({
                operationName: 'core_template_nosession',
                variables: {
                    component: component,
                    name: name,
                    theme: config.theme
            ...
            promise.then(
                function(data) {
                    var templateSource = data.template;
         ...
         * @return {Promise} resolved on completion of the autointialise script
         */
        var runTemplateJS = function(source) {
            if (source !== undefined && source.trim() !== '') {
                M.util.js_pending('core-templates-customjs');
                source += ";M.util.js_complete('core-templates-customjs');";
