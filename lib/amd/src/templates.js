...
define([ 'core/mustache',
    ...
    function(mustache, $, ajax, str, notification, coreurl, config, storage, event, flexicon, log) {
        ...
        /**
         * Scan's for widgets, initialising them when possible
         *
         * @returns {Promise} ES6 promise which resolves when all widgets are initialised
         */
        var widgetScan = function() {
            var initingObjects = [];
            var elements = document.querySelectorAll('[data-x-enhanced="false"][data-x-widget]');

            /**
             * Creates a function suitable for adding as an agument to the Promise constructor
             *
             ...
             */
            var enhanceWidget = function(element) {
                var amdModule = element.getAttribute('data-x-amd'),
                    widget = element.getAttribute('data-x-widget');
                element.setAttribute('data-x-enhanced', 'enhancing');

                /**
                 * Promise function
                 *
                 * @param {Function} resolve called when the node is enhanced
                 * @param {Function} reject called if there is anything that has gone wrong when enhancing the node
                 */
                var promiseFunc = function(resolve, reject) {
                    require([amdModule], function(widgetJS) {
                        try {
                            widgetJS.init(element).then(function() {
                                element.setAttribute('data-x-enhanced', 'true');
                                resolve();
                            }).catch(reject);
                        } catch (e) {
                            reject(e);
                        ...
                        reject('Failed to load AMD module "' + amdModule + '" for widget "' + widget + '"');
                    });
                };
            ...
            for (var element = 0; element < elements.length; element++) {
                var promiseFunc = enhanceWidget(elements[element]);
        ...
        var runTemplateJS = function(source) {
            M.util.js_pending('js-template');
            if (typeof source !== 'undefined' && source.trim() !== '') {
                var newscript = $('<script>').html(source);
                $('head').append(newscript);
            }
            widgetScan().then(function() {
                M.util.js_complete('js-template');
            }, function(message) {
                log.error(message);
            });
        };
