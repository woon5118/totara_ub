...
define([ 'core/mustache',
    ...
    var doRender = function(templateSource, context, themeName) {
        ...
        var templatepartialsloaded = false;
        ...
        var partialHelper = function(name) {
            ...
            if (templatepartialsloaded && template === '') {
                require(['core/log'], function(log) {
                    log.error('partial "' + name + '" not in cache on second pass');
                });
            }
        ...
        // Load the partials. This will place the partials in the cache so partialHelper can
        // return immediately on the serious run.
        mustache.render(templateSource, context, partialHelper);
        var partialsloaded = $.Deferred();
        if (partialPromises.length > 0) {
            partialsloaded = $.when.apply($, partialPromises);
        } else {
            partialsloaded.resolve();
        }

        partialsloaded.done(function() {
