...
class scss {
    ...
    public function get_compiled_css(string $component): string {
        ...
        if ($import_data->cssvars_legacy_imports) {
            $css_realvars_legacy = $this->compile($this->build_import_code($import_data->cssvars_legacy_imports), false);
            $legacy_var_values = $this->options->get_cssvars()->get_custom_property_values($css_realvars_legacy);
        }

        $output = $this->compile($this->build_import_code($import_data->imports), $this->options->get_sourcemap_enabled());
    ...
    private function get_imports(string $component): object {
        ...
        $component_files = $component !== 'theme'
            ? $this->get_component_tui_scss_files($component)
            : ['variables' => [], 'scss' => []];
        ...
        foreach ($def_file_sets as $def_files) {
            foreach ($def_files['variables'] as $def_file) {
                if ($this->choose_build_file($def_file)) {
        ...
        $source_file_sets = $component === 'theme' ? $themes_files : [$component_files];
        foreach ($source_file_sets as $source_files) {
            foreach ($source_files['variables'] as $source_file) {
                if ($this->choose_build_file($source_file)) {
    ...
    private function resolve_import(string $url): ?string {
        ...
        if (substr($url, 0, 18) == 'internal_absolute:') {
            $path = $this->choose_build_file(substr($url, 18));
            if ($path) {
                return $path;
            }
            return null;
        ...
        foreach ($urls as $url) {
            $path = $component_dir . '/tui/build/styles/' . $url . '.scss';
            $path = $this->choose_build_file($path);
            if ($path) {
                return $path;
            }
        }
    ...
    protected function get_component_tui_scss_files(string $component): array {
        $cssfiles = ['scss' => [], 'variables' => []];
        $type_files = [
            'scss' => ['bundle' => '/tui/build/tui_bundle.scss'],
            'variables' => ['_variables' => '/tui/build/styles/_variables.scss']
        ];
        $dir = $this->get_component_directory($component);
        foreach ($type_files as $type => $files) {
            foreach ($files as $name => $file) {
                $path = $dir . $file;
                $path = $this->choose_build_file($path);
                if ($path) {
                    $cssfiles[$type][$name] = $path;
                }
            }
        }
        return $cssfiles;
    ...
    /**
     * {@see \core_output_choose_build_file()}
     */
    protected function choose_build_file(string $file): ?string {
        return ($this->options->get_choose_build_file())($file);
    }

    /**
     * {@see \core_component::get_component_directory()}
     */
    protected function get_component_directory(string $component): ?string {
        return ($this->options->get_get_component_directory())($component);
    }
}
