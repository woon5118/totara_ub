...
Feature: Gradebook calculations for calculated grade items before the fix 20150627
  ...
  Scenario: The max grade for a category item, with a calculation using Natural aggregation, can be changed
    ...
    And the following should exist in the "user-grade" table:
      ...
      | Calculated gradeCalc cat totalInclude empty grades. | 100.00 %          | 37.50  | 0–100 | 37.50 %    | -                            |
      | NaturalCourse total                                 | -                 | 37.50  | 0–100 | 37.50 %    | -                            |
  ...
  Scenario: Changing max grade for a category item with a calculation that has existing grades will display the same points with the new max grade values immediately.
    ...
    And the following should exist in the "user-grade" table:
      ...
      | Calculated gradeCalc cat totalInclude empty grades. | 100.00 %          | 32.50  | 0–100 | 32.50 %    | -                            |
      | NaturalCourse total                                 | -                 | 32.50  | 0–100 | 32.50 %    | -                            |
  ...
  Scenario: Values in calculated grade items are not always out of one hundred
    ...
    Then the following should exist in the "user-grade" table:
      | Grade item                | Calculated weight | Grade  | Range | Percentage | Contribution to course total |
      | Manual itemgrade item 1   | 50.00 %           | 75.00  | 0–100 | 75.00 %    | 37.50 %                      |
      | Calculated gradecalc item | 50.00 %           | 37.50  | 0–100 | 37.50 %    | 18.75 %                      |
      | NaturalCourse total       | -                 | 112.50 | 0–200 | 56.25 %    | -                            |
    And I navigate to "Gradebook setup" node in "Grade administration > Setup"
    And I set the following settings for grade item "calc item":
      | Maximum grade | 40 |
    And I follow "Grader report"
    ...
    And the following should exist in the "user-grade" table:
      | Grade item                | Calculated weight | Grade  | Range | Percentage | Contribution to course total |
      | Manual itemgrade item 1   | 50.00 %           | 65.00  | 0–100 | 65.00 %    | 32.50 %                      |
      | Calculated gradecalc item | 50.00 %           | 32.50  | 0–100 | 32.50 %    | 16.25 %                      |
      | NaturalCourse total       | -                 | 97.50  | 0–200 | 48.75 %    | -                            |
