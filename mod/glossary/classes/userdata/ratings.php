 ...
 */
class ratings extends \totara_userdata\userdata\item {
    /**
    ...
    protected static function purge(target_user $user, \context $context) {
        ...
        $sql = "SELECT DISTINCT r.contextid, r.itemid, g.id AS glossaryid, g.assessed, cm.idnumber AS cmidnumber, cm.id AS cmid
                  FROM {rating} r
                  JOIN {glossary_entries} e ON e.id = r.itemid 
                 $join
                 WHERE r.userid = :userid AND r.component = 'mod_glossary' AND r.ratingarea = 'entry' AND r.contextid = ctx.id";
        $ratings = $DB->get_recordset_sql($sql, array('userid' => $user->id));
        $glossaries = array();
        foreach ($ratings as $rating) {
            ...
            $rm->delete_ratings((object)array('contextid' => $glossarycontext->id, 'component' => 'mod_glossary', 'ratingarea' => 'entry', 'userid' => $user->id));
        }
        $ratings->close();
        ...
        foreach ($glossaries as $gid => $cmidnumber) {
            $glosssary = $DB->get_record('glossary', array('id' => $gid));
    ...
    protected static function export(target_user $user, \context $context) {
        ...
        $join = self::get_activities_join($context, 'glossary', 'e.glossaryid', 'g');
        $sql = "SELECT r.*, g.id AS glossaryid
                  FROM {rating} r
                  JOIN {glossary_entries} e ON e.id = r.itemid 
                 $join
                 WHERE r.userid = :userid AND r.component = 'mod_glossary' AND r.ratingarea = 'entry' AND r.contextid = ctx.id
        ...
        foreach ($ratings as $rating) {
            $value = $rating->rating;
            if ($rating->scaleid < 0) {
                $scalerecord = $DB->get_record('scale', array('id' => abs($rating->scaleid)));
                if ($scalerecord) {
