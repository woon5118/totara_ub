...
use mod_perform\webapi\resolver\query\user_grouping_positions;
use mod_perform\webapi\resolver\type\user_grouping_position;

use totara_webapi\graphql;

/**
 * @coversDefaultClass user_grouping_positions.
 *
 * @group perform
 *
 ...
 */
class mod_perform_webapi_query_user_grouping_positions_testcase extends advanced_testcase {
    /**
     * @covers ::resolve
     */
    public function test_find(): void {
        ...
        $actual_groups = user_grouping_positions::resolve([], $context);
        $this->assertCount(count($groups_by_id), $actual_groups, 'wrong retrieve count');

        foreach ($actual_groups as $group) {
            $expected = $groups_by_id[$group->id] ?? null;
            $this->assertNotNull($expected, 'unknown group retrieved');
            $this->assertEquals($expected->shortname, $group->shortname, 'wrong short name');
            $this->assertEquals($expected->fullname, $group->fullname, 'wrong full name');
            $this->assertEquals($expected->description, $group->description, 'wrong description');
     ...
     * Generates test data.
     *
     * @param int $count no of positions to generate.
     *
     * @return array[int=>stdClass] the mapping of position ids to
     *         positions.
     */
    ...
    private function graphql_return(\stdClass $group, execution_context $context): array {
        $resolve = function (string $field) use ($group, $context) {
            return user_grouping_position::resolve($field, $group, [], $context);
        };
    ...
    private function get_webapi_context(): execution_context {
        return execution_context::create('ajax', 'mod_perform_user_grouping_positions');
    }
}
