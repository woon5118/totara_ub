...
use mod_perform\webapi\resolver\query\user_grouping_cohorts;
use mod_perform\webapi\resolver\type\user_grouping_cohort;

use totara_webapi\graphql;

/**
 * @coversDefaultClass user_grouping_cohorts.
 *
 * @group perform
 *
 ...
 */
class mod_perform_webapi_query_user_grouping_cohorts_testcase extends advanced_testcase {
    /**
     * @covers ::resolve
     */
    public function test_find(): void {
        ...
        $actual_groups = user_grouping_cohorts::resolve([], $context);
        $this->assertEquals(count($groups_by_id), $actual_groups->count(), 'wrong retrieve count');

        foreach ($actual_groups as $group) {
            $expected = $groups_by_id[$group->id] ?? null;
            $this->assertNotNull($expected, 'unknown group retrieved');
            $this->assertEquals($expected->name, $group->name, 'wrong name');
     ...
     *
     * @return array[int=>stdClass] the mapping of cohort ids to cohorts.
     */
    private function setup_env(int $count=10): array {
        ...
        foreach (range(0, $count - 1) as $unused) {
            $group = $this->getDataGenerator()->create_cohort();
    ...
    private function graphql_return(\stdClass $group, execution_context $context): array {
        $resolve = function (string $field) use ($group, $context) {
            return user_grouping_cohort::resolve($field, $group, [], $context);
        };
    ...
    private function get_webapi_context(): execution_context {
        return execution_context::create('ajax', 'mod_perform_user_grouping_cohorts');
    }
}
