...
class mod_perform_job_assignment_start_date_resolver_testcase extends advanced_testcase {
    ...
    public function test_resolve() {
        ...
        $job_assignment_start_date_resolver->set_parameters(
            ...
            [$data['user1']->id],
            [$data['user1-job1']->id, $data['user1-job2-with-start']->id]
        );

        $start_date_job1 = $job_assignment_start_date_resolver->get_start_for($data['user1']->id,
            $data['user1-job1']->id);
        $this->assertNull($start_date_job1);

        $expected = strtotime("+1 day", 123);
        $start_job2 = $job_assignment_start_date_resolver->get_start_for($data['user1']->id,
            $data['user1-job2-with-start']->id);
        $this->assertSame($expected, $start_job2);

        $expected = strtotime("+2 day", 123);
        $end_job2 = $job_assignment_start_date_resolver->get_end_for($data['user1']->id,
            $data['user1-job2-with-start']->id);
    ...
    public function test_resolve_exception() {
        ...
        $start_job3 = $job_assignment_start_date_resolver->get_start_for($data['user1']->id,
            $data['user2-job3']->id);
    }
