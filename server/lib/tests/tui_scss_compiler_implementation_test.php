...
class core_tui_scss_compiler_implementation_testcase extends basic_testcase {
     ...
     *
     * @return testable_tui_scss_compiler
     */
    private function create_compiler() {
        ...
        $transform_mock_content = $this->getMockForAbstractClass(\core\tui\scss\transforms\transform::class);
        $transform_mock_content->expects($this->any())
            ->method('execute')
            ->will($this->returnCallback(function (\core\tui\scss\transform_resource $resource) {
                $code = $resource->get_code();
                $resource->set_code("sample { content: '" . str_replace(["'", "\n"], ["\\'", '\A'], $code) . "'; }");
            }));

        $transform_mock_prefix = $this->getMockForAbstractClass(\core\tui\scss\transforms\transform::class);
        ...
        $compiler->register_transform('definitions_only', new \core\tui\scss\transforms\definitions_only());
        $compiler->register_transform('output_only', new \core\tui\scss\transforms\output_only());
