...
$currentlanguage = required_param('currentlanguage', PARAM_LANG);
...
$PAGE->set_url('/blocks/totara_report_graph/ajax_graph.php', array('blockid' => $blockid, 'currentlanguage' => $currentlanguage));
...
if (empty($block->configdata) || !$totara_report_graph || !$totara_report_graph->user_can_view()) {
    die;
}
...
// Use the same language as the page accessing this script.
$SESSION->lang = $currentlanguage;

$config = unserialize(base64_decode($block->configdata));
if ($config) {
    $data = \block_totara_report_graph\util::get_chart_data($block, (object)$config);
    if ($data) {
...
echo json_encode(['error' => get_string('errorrendergraph', 'block_totara_report_graph')]);
