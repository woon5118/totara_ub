...
use totara_engage\upload\upload;
...
class totara_engage_upload_testcase extends advanced_testcase {
    /**
    ...
    public function test_save_draft() {
        global $DB;
        $draftitemid = $this->generate_draft();
        upload::save_draft($draftitemid, context_system::instance()->id, 'totara_engage', 'test', 42);
        ...
        foreach($records as $record) {
            ...
            $this->assertEquals('totara_engage', $record->component);
    ...
    public function test_save_draft_not_overwrite_existing() {
        global $DB;
        $this->markTestSkipped("It fails on full run only. Reason not known, but looks like something overrides permission of the file prior this test.");
        return;
        $draftitemid = $this->generate_draft();
        upload::save_draft($draftitemid, context_system::instance()->id, 'totara_engage', 'test', 42);
        $draftitemidnew = $this->generate_draft('hocus.txt');
        upload::save_draft($draftitemidnew, context_system::instance()->id, 'totara_engage', 'test', 42);
        $records = $DB->get_records('files', ['itemid' => 42]);
