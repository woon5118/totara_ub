...
$PAGE->set_url('/totara/engage/reporting/workspace_report.php');
...
$PAGE->navbar->add(get_string('engagedworkspace', 'totara_engage'));

$config = (new rb_config())->set_sid($sid);
$report = reportbuilder::create_embedded('engagedworkspace', $config);

$PAGE->set_button($report->edit_button() . $PAGE->button);

$heading = get_string('engagedworkspace', 'totara_engage');
...
echo $output->heading(get_string('engagedworkspace', 'totara_engage') . ': ' . $output->result_count_info($report));
