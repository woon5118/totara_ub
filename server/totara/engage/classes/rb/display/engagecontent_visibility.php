 ...
 */
final class engagecontent_visibility extends base {
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report): string {
        if(access::is_private((int)$value)) {
            return static::to_plaintext(get_string('visibility_private', 'rb_source_engagecontent'));
        }
        
        if (access::is_public((int)$value)) {
            return static::to_plaintext(get_string('visibility_public', 'rb_source_engagecontent'));
        }
    
        if (access::is_restricted((int)$value)) {
            return static::to_plaintext(get_string('restricted', 'totara_engage'));
        }
