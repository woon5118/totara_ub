...
/**
 * Display class intended for reported name
 */
final class engagecontent_name extends base {
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report): string {
        ...
        $url = null;
        if ($component === 'engage_article') {
           $article = article::from_resource_id($extra->resourceid);
           $url = $article->get_url();
        } else if ($component === 'engage_survey') {
            $survey = survey::from_resource_id($extra->resourceid);
            $url  = $survey->get_url();
        }
