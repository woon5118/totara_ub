...
/**
 * Manage tenant membership and migration to non-member.
 */
final class member_manage extends \totara_form\form {
    /**
     * Form definition.
     *
     * @return void
    ...
    public function definition() {
        ...
        $warningstr = get_string('migrationtononmemberwarning', 'totara_tenant');
        $warning = new \totara_form\form\element\static_html('warning', '', $warningstr);
        $this->model->add($warning);
        $this->model->add_clientaction(new \totara_form\form\clientaction\hidden_if($warning))->not_equals($tenant, 0);
