...
/**
 * Users with roles in tenant context
 */
final class tenant_user_managers extends base {
    /** @var array static cache */
    protected static $used_roles;

    /**
     * All roles assigned at tenant contexts.
     *
     * @param bool $usecache
     * @return array
     */
    public static function get_used_roles(bool $usecache = true): array {
        ...
        $sql = 'SELECT DISTINCT r.*
                  ...
                  JOIN "ttr_context" ctx ON ctx.id = ra.contextid AND ctx.contextlevel = :tenantlevel
              ORDER BY r.sortorder';
        $params = ['tenantlevel' => CONTEXT_TENANT];
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report) {
        ...
        $params = ['contextid' => $tenantcontext->id];
        ...
        if ($format === 'html') {
            if (has_capability('moodle/role:assign', $tenantcontext)) {
                if ($perrole or $DB->record_exists('cohort_members', ['cohortid' => $tenant->cohortid])) {
                    $url = new \moodle_url('/admin/roles/assign.php', array('contextid' => $tenantcontext->id));
