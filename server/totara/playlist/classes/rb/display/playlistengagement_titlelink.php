 ...
 */
final class playlistengagement_titlelink extends base {
    /**
     * Handles the display
     *
     * @param string $value
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report): string {
        ...
        if ((int)$extra->deleted === 1 || (int)$extra->suspended === 1) {
            return $value;
        }

        $id = $extra->id;
        $playlist = playlist::from_id((int)$id);

        $value = format_string::display($value, $format, $row, $column, $report);
        return html_writer::link($playlist->get_url(), $value);
    }
