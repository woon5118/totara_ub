...
/**
 * A dispatcher for the signin command.
 */
class signin_request implements dispatchable, describable {
    /**
     * @inheritDoc
     */
    ...
    public function get_description(): lang_string {
        return new lang_string('botfw:help_signin', 'totara_msteams');
    }
    ...
    public function dispatch(bot $bot, activity $activity): void {
        try {
            ...
            $bot->reply_text_to($activity, get_string('botfw:msg_signin_already', 'totara_msteams', $name));
            ...
            $message = builder::message()
                ...
                ->add_attachment(
                    builder::signin_card()
                        ->text(get_string('botfw:msg_signin', 'totara_msteams'))
                        ->add_button(builder::action()
                            ->signin(get_string('botfw:msg_signin_button', 'totara_msteams'), $url)
                            ->build())
                        ->build());
