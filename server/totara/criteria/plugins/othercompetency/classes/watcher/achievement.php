...
class achievement {
    ...
    public static function updated_bulk(competency_achievement_updated_bulk $hook) {
        ...
        // Find all criteria items use this competency as other competency (i.e. find all criteria with a 'competency' item
        ...
        $other_competency_id = $hook->get_competency_id();
        $user_ids = $hook->get_user_ids();
        if (empty($user_ids)) {
            return;
        }

        competency_item_helper::achievement_updated($user_ids, $other_competency_id, 'othercompetency');
    }
