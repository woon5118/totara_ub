...
final class rb_source_topicusage extends rb_base_source {
            ...
            new rb_join(
                ...
                '
                   (SELECT COUNT(ti.itemid) AS totalusage, ti.component, ti.tagid 
                    FROM {tag_instance} ti GROUP BY ti.component, ti.tagid)
                ',
    ...
    private function get_filter_for_components(): rb_filter_option {
        ...
        $sql = 'SELECT component FROM "ttr_tag_area" WHERE tagcollid = ?';
        $components = $DB->get_fieldset_sql($sql, [$CFG->topic_collection_id]);
