...
/**
 * Display class intended to convert a course name into an expanding link.
 * When exporting, only the user's full name is displayed (without link)
 *
 * @author Simon Player <simon.player@totaralearning.com>
 * @package totara_reportbuilder
 */
class course_expand extends base {
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report) {
        ...
        $alturl = new \moodle_url('/course/view.php', array('id' => $extrafields->course_id));

        // Serialize the data so that it can be passed as a single value.
        $paramstring = http_build_query(array('expandcourseid' => $extrafields->course_id), '', '&');
