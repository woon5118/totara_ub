...
Feature: Manage availability conditions
  ...
  Scenario: Hide and show conditions
    ...
    Then "[title=Hide]" "css_element" should exist in the "Restriction by date" "table_row"

    # Click the icon. It should toggle to hidden (title=Show).
    And I click on "[title=Hide]" "css_element" in the "Restriction by date" "table_row"
    And "[title=Show]" "css_element" should exist in the "Restriction by date" "table_row"

    # Toggle it back to visible (title=Hide).
    And I click on "[title=Show]" "css_element" in the "Restriction by date" "table_row"
    And "[title=Hide]" "css_element" should exist in the "Restriction by date" "table_row"

    # OK, toggling works. Set the grade one to Hide and we'll go see if it actually worked.
    And I click on "[title=Hide]" "css_element" in the "Restriction by grade" "table_row"
