...
Feature: An admin can update courses enrolments using a CSV file
  ...
  @javascript
  Scenario: Updating an enrolment method and not specifying enrolment status
    Given I am on "Course 1" course homepage
    And I navigate to "Users > Enrolment methods" in current page administration
    Then "Enable" "link" should exist in the "Guest access" "table_row"
    And I navigate to "Courses > Upload courses" in site administration
    And I upload "admin/tool/uploadcourse/tests/fixtures/enrolment_no_disable.csv" file to "File" filemanager
    And I set the field "Upload mode" to "Only update existing courses"
    And I set the field "Update mode" to "Update with CSV data only"
    And I set the field "Allow deletes" to "Yes"
    And I click on "Preview" "button"
  ...
  @javascript
  Scenario: Creating an enrolment method and not specifying enrolment status
    Given I am on "Course 1" course homepage
    And I navigate to "Users > Enrolment methods" in current page administration
    Then "Enable" "link" should exist in the "Guest access" "table_row"
    And I click on "Delete" "link" in the "Guest access" "table_row"
