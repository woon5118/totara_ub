...
class mod_perform_delete_user_testcase extends advanced_testcase {
    ...
    public function test_deleting_user_closes_only_open_participant_instances() {
        self::setAdminUser();
        ...
        $this->assertEquals(participant_instance_availability_not_applicable::get_code(), $participant_instance->availability);

        foreach ($participant_instance->participant_sections()->get() as $participant_section) {
            $this->assertEquals(participant_section_availability_not_applicable::get_code(), $participant_instance->availability);
        }
