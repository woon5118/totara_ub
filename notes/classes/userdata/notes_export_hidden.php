 ...
 */
class notes_export_hidden extends item {
    ...
    /**
     * Gets the notes in the context that the user cannot see.
    ...
    private static function get_notes(target_user $user, \context $context): array {
        ...
        foreach ($courseids as $courseid) {
            if (has_capability('moodle/notes:view', \context_course::instance($courseid), $user)) {
                // If the user has notes:view they can't see personal notes they didn't create.
                $personalnotes = note_list($courseid, $user->id, NOTES_STATE_DRAFT);
                $hiddenpersonalnotes = array_filter($personalnotes, function($note) use ($user) {
                    return $note->usermodified != $user->id;
                });
                $notes += $hiddenpersonalnotes;
            } else {
                // If they dont have notes:view then they can not see any course or personal notes.
                $notes += note_list($courseid, $user->id, NOTES_STATE_PUBLIC);
                $notes += note_list($courseid, $user->id, NOTES_STATE_DRAFT);
            }
        }
