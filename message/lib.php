...
function message_count_unread_messages($user1=null, $user2=null) {
    ...
    if (!empty($user2)) {
        return $DB->count_records_sql("SELECT COUNT('id')
                                         FROM {message} m
                                    LEFT JOIN {message_metadata} md ON md.messageid = m.id
                                        WHERE m.useridto = ? AND m.useridfrom = ? AND m.notification = 0 AND m.timeusertodeleted = 0
                                              AND md.id IS NULL",
        ...
        return $DB->count_records_sql("SELECT COUNT('id')
                                         FROM {message} m
                                    LEFT JOIN {message_metadata} md ON md.messageid = m.id
                                        WHERE m.useridto = ? AND m.notification = 0 AND m.timeusertodeleted = 0
                                              AND md.id IS NULL",
            array($user1->id));
    }
}
 ...
 *
 * Totara: this function is used to return a page type
 *         for adding blocks to the message/index page.
 *
