...
Utilizzare admin/cli/install_database.php per installare Totara.';
$string['clialreadyinstalled'] = 'Il file config.php è già presente.
Utilizzare admin/cli/install_database.php per aggiornare Totara.';
$string['cliinstallheader'] = 'Programma di installazione Totara {$a} via linea di comando';
...
<p>Tale limite potrà causare problemi nel funzionamento di Totara, specialmente se usi molti moduli di attività con molti utenti.</p>
...
Questo consentirà a Totara di impostare in autonomia il limite di memoria.</li>
...
<li>Su alcuni server con il PHP è possibile creare un file .htaccess nella cartella di Totara contenente questa linea:
...
$string['pathssubadmindir'] = 'Alcuni web host utilizzano la cartella /admin come URL di accesso a pannelli di controllo od altre funzioni particolari. Tuttavia questo nome coincide con il nome della cartella che Totara utilizza per i propri file di amministrazione. Per evitare conflitti, è possibile specificare un nome alternativo per la cartella Admin di Totara. Ad esempio:<p><b>moodleadmin</b></p>
Tutti i link che puntano ai file di amministrazione di Totara terranno conto di questa variazione.';
$string['pathssubdataroot'] = '<p>La cartella dove Totara memorizzerà i file caricati dagli utenti.</p>
...
$string['pathssubdirroot'] = '<p>Percorso assoluto della cartella dove si trova il codice di Totara.</p>';
$string['pathssubwwwroot'] = '<p>L\'iIndirizzo web per raggiungere a Totara, in altre parole l\'indirizzo che gli utenti inseriranno nella barra degli indirizzi del proprio browser per accedere a questa installazione.</p>
<p>Non è possibile accedere alla stessa installazione Totara usando più di un indirizzi web. Se il tuo sito usa più indirizzi web, devi configurare dei re-indirizzamenti permanenti per tutti gli altri indirizzi web.</p>
...
$string['phpversionhelp'] = '<p>Totara necessita come minimo della versione 4.3.0 o 5.1.0 del PHP. (La versione 5.0.x soffre di problemi noti)</p>
...
$string['welcomep30'] = 'La release di <strong>{$a->installername}</strong> include l\'applicazione per creare l\'ambiente necessario a far girare <strong>Totara</strong>:';
$string['welcomep40'] = 'Il pacchetto include anche <strong>Totara {$a->moodlerelease} ({$a->moodleversion})</strong>.';
...
$string['welcomep60'] = 'Le prossime pagine ti guideranno attraverso semplici passi per installare e configurare <strong>Totara</strong> nel tuo computer. Puoi utilizzare le impostazioni di default oppure modificarle per adeguarle alle tue esigenze.';
$string['welcomep70'] = 'Fai click sul pulsante "Avanti" per continuare l\'installazione di <strong>Totara</strong>.';
