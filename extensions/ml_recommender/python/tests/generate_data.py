...
class GenerateData:
    """
    This is a conceptual representation of the process of generating fake data for the recommender
    """
    def __init__(self, n_tenants=5, n_users=10, n_items=20, users_interacted=0.95, items_interacted=1.0):
        """
        Class constructor method
        :param n_tenants: Number of tenants to be created as on `tenants.csv`, defaults to 5
        ...
        :param users_interacted: The percentage of users who interacted with any item (0 < `users_interacted` <= 1),
            defaults to 0.95
        :type users_interacted: float, optional
        :param items_interacted: The percentage of items that got interacted by any user (0 < `items_interacted` <= 1),
            defaults to 1
        ...
        self.item_id = ['engage_article' + str(x) for x in range(1, (article_n + 1))] + \
        ...
        if 0.0 < users_interacted < 1.0:
            self.user_id_inter = random.sample(population=self.user_id, k=int(users_interacted * n_users))
        if 0 < items_interacted < 1.0:
            self.item_id_inter = random.sample(population=self.item_id, k=int(items_interacted * n_items))
    ...
    def get_items(self):
        ...
        item_types = pd.get_dummies(data=[''.join(c for c in i if not c.isdigit()) for i in self.item_id])
        item_types = item_types.rename(index=dict(zip(range(len(self.item_id)), self.item_id)))
    ...
    def get_interactions(self):
        ...
        df_items = pd.DataFrame({'key': [1] * len(self.item_id_inter), 'item_id': self.item_id_inter})
        ...
        interactions['timestamp'] = [random.randint(a=time_start, b=time_now) for _ in range(inter_len)]
