...
class core_user_profilelib_testcase extends advanced_testcase {
    ...
    public function test_profile_has_required_custom_fields_set() {
        ...
        $this->assertTrue(user_not_fully_set_up($hermione));
        $this->assertTrue(user_not_fully_set_up($hermione));

        // When confirming remote MNet users, we do not have custom fields available.
        $roamingharry = mnet_strip_user($harry, ['id', 'firstname', 'lastname', 'email']);
        $roaminghermione = mnet_strip_user($hermione, ['id', 'firstname', 'lastname', 'email']);

        $this->assertTrue(user_not_fully_set_up($roamingharry));
        $this->assertTrue(user_not_fully_set_up($roamingharry));
        $this->assertTrue(user_not_fully_set_up($roaminghermione));
        $this->assertTrue(user_not_fully_set_up($roaminghermione));
    }
