...
class systemaccess extends \totara_userdata\userdata\item {
    ...
    public static function get_fullname_string() {
        return ['userdataitem-user-systemaccess', 'core'];
    }
    ...
    protected static function purge(target_user $user, \context $context) {
        ...
        if (!empty($user->lastip) && $user->lastip != '0.0.0.0') {
            $updates['lastip'] = '';
        }

        if (!empty($updates)) {
            $updates['id'] = $user->id;
            $DB->update_record('user', (object)$updates);
        }

        $DB->delete_records('user_lastaccess', ['userid' => $user->id]);

        // NOTE: do not trigger events, user must be already deleted.
    ...
    protected static function export(target_user $user, \context $context) {
        ...
        $export->data = [
            'firstaccess' => !empty($user->firstaccess) ? $user->firstaccess : '',
            'lastaccess' => !empty($user->lastaccess) ? $user->lastaccess : '',
            'lastlogin' => !empty($user->lastlogin) ? $user->lastlogin : '',
            'currentlogin' => !empty($user->currentlogin) ? $user->currentlogin : '',
            'lastip' => (!empty($user->lastip) && $user->lastip != '0.0.0.0') ? $user->lastip : '',
            'courses' => $courses
        ];
    ...
    protected static function count(target_user $user, \context $context) {
        ...
        $count = [
            intval(!empty($user->firstaccess)),
            intval(!empty($user->lastaccess)),
            intval(!empty($user->lastlogin)),
            intval(!empty($user->currentlogin)),
            intval(!empty($user->lastip) && $user->lastip != '0.0.0.0'),
            $DB->count_records('user_lastaccess', ['userid' => $user->id])
        ];
